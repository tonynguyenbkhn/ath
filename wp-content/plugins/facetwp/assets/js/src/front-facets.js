(function($) {
    ...
    FWP.hooks.addFilter('facetwp/selections/dropdown', function(output, params) {
        var choices = [];
        var active = params.el.nodes.find(node => !node.classList.contains('facetwp-ignore')); // If there are duplicate Dropdown facets, use the one without facetwp-ignore
        var $item = $(active).find('.facetwp-dropdown');
        if ($item.len()) {
            var dd = $item.nodes[0];
    ...
    $f(document).on('change', '.facetwp-type-dropdown select', function() {
        ...
        $facet.find('.facetwp-dropdown [selected=""]').each(function() {
            if ($(this).val() != val) {
        ...
        if ('' !== val) {
            ...
            $facet.find('.facetwp-dropdown option[value="' + val + '"]').attr("selected","");
        }
        FWP.autoload();
    });
    ...
    FWP.hooks.addFilter('facetwp/selections/sort', function(output, params) {
        var choices = [];
        var active = params.el.nodes.find(node => !node.classList.contains('facetwp-ignore')); // If there are duplicate Sort facets, use the one without facetwp-ignore
        var $item = $(active).find('select');
        if ($item.len()) {
            var dd = $item.nodes[0];
            var val = dd.options[dd.selectedIndex].value;
            var text = dd.options[dd.selectedIndex].text;
    ...
    $().on('change', '.facetwp-type-sort select', function() {
        ...
        $facet.find('.facetwp-type-sort select [selected=""]').each(function() {
            if ($(this).val() != val) {
                $(this).removeAttr("selected");
            }
        });
        ...
        if ('' !== val) {
            FWP.frozen_facets[facet_name] = 'hard';
            $facet.find('.facetwp-type-sort select option[value="' + val + '"]').attr("selected","");
        }
        FWP.autoload();
    });
    ...
    FWP.hooks.addAction('facetwp/loaded', function() {
        ...
        $('.facetwp-hide-empty').each(function() {
            ...
            let all_facets = ! FWP.loaded && ! FWP.is_bfcache && $.isset(FWP_HTTP.url_vars) ? Object.assign(FWP.facets, FWP_HTTP.url_vars): FWP.facets;
            let all_empty = facets.every(val => 'undefined' === typeof all_facets[val] || all_facets[val].length < 1);

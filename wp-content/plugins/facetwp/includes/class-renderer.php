...
class FacetWP_Renderer
{
    ...
    function get_query_args() {
        ...
        if ( apply_filters( 'facetwp_template_use_archive', false ) ) {
            ...
            if ( $main_query->is_archive || $main_query->is_search ) {
                ...
                elseif ( $main_query->is_author ) {
                    $defaults['author'] = $main_query->get( 'author' );
                }
            ...
            elseif ( ! empty( $this->http_params['archive_args'] ) ) {
                foreach ( $this->http_params['archive_args'] as $key => $val ) {
                    if ( in_array( $key, [ 'cat', 'tag_id', 'taxonomy', 'term', 'author', 's' ] ) ) {
                        $defaults[ $key ] = $val;
                    }
                }
            }
    ...
    /**
     * Determine if we are on a WP archive template (for Debug Mode)
     * @since 4.5.0
     */
    function is_archive() {

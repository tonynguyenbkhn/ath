...
class FacetWP_Indexer
{
    ...
    function index( $post_id = false ) {
        ...
        $this->cancel_indexing();  // for cancelling another index on new request
        ...
        foreach ( $post_ids as $counter => $post_id ) {
            ...
            if ( $this->index_all ) {
                if ( 0 == ( $counter % $this->chunk_size ) ) {
                    ...
                    $this->cancel_indexing(true);  //for cancelling indexing in progress
     ...
     * This can be triggered by "facetwp_index_row" to handle multiple values
     * @since 1.2.5
     */
    ...
    function manage_temp_table( $action = 'create' ) {
        ...
        elseif ( 'delete' == $action ) {
            ...
            // Cleanup options as late as possible to prevent 'The index table is empty'
    ...
    /**
     * Check for facetwp_indexing_cancelled in options and
     * run cancel and cleanup for indexing
     * @param bool $abort
     * @return bool
     * @since 4.x
     */
    function cancel_indexing( $abort = false ) {

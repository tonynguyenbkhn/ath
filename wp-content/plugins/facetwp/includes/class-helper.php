...
final class FacetWP_Helper
{
    ...
    function load_settings( $last_index = false ) {
        ...
        foreach ( $facets as $facet ) {
            ...
            if ( ! $is_db_based || empty( $tmp_facets[ $name ] ) ) {
                ...
                if ( in_array( $facet['type'], array_keys( $this->facet_types ) ) ) {
                    ...
                    if ( isset($facet['_missing']) ) unset($facet['_missing']);
                    $tmp_facets[ $name ] = $facet;
                } else {
                    $facet['_missing'] = true;
                    $tmp_facets_missing[ $name ] = $facet;
                }
            }
        }
        ...
        $settings['facets_missing'] = array_values( $tmp_facets_missing );
     ...
     * @param string $include 'all' or '', all returns missing facets also
     * @return array
     */
    function get_facets( $include = '' ) {
        $facets = $this->settings['facets'];
        if ( 'all' === $include ) {
            $facets = array_merge( $this->settings['facets_missing'], $facets );
        }
        return $facets;
    }
     ...
     * @param string $include 'all' or '', all searches for missing facets also
     * @return mixed An array of facet info, or false
     */
    function get_facet_by_name( $facet_name, $include = '' ) {
        foreach ( $this->get_facets( $include ) as $facet ) {
    ...
    /**
     * Explode input if string, return or discard input if input is array
     * 
     * @param string $separator - boundary string for explode
     * @param mixed $input - should be a string but could be an array
     * @param bool $keep - whether to discard the input if its an array
     * @return array
     * @since 4.5.0
     */
    function explode( $separator, $input, $keep = false ) {
        $output = []; 

        if ( is_array( $input ) && $keep != false ) {
            $output = $input;
        }
     ...
     * @since 2.7.5
     */
    function format_number( $num, $sep_decimal = '', $sep_thousands = '' ) {
        $sep_decimal = empty( $sep_decimal ) ? $this->get_setting( 'decimal_separator' ) : $sep_decimal;
        $sep_thousands = empty( $sep_thousands ) ? $this->get_setting( 'thousands_separator' ) : $sep_thousands;
    ...
    function get_data_sources( $context = 'default' ) {
        ...
        if ( ! empty( $this->data_sources ) ) {
            ...
            // Get meta keys to exclude from sql query
            // use may cause query to be slower
            $excluded_fields_like = apply_filters( 'facetwp_excluded_custom_fields_like', [] );

            // Get taxonomies
            $taxonomies = get_taxonomies( [], 'object' );

            // Get custom fields with partial match exclusion
            $sql = "SELECT DISTINCT meta_key FROM {$wpdb->postmeta}";
            if ( !empty( $excluded_fields_like ) ) {
                $sql .= " WHERE ";
                $clauses = [];
                foreach ( $excluded_fields_like as $ex ) {
                    $clauses[] = $wpdb->prepare( "meta_key NOT LIKE %s", '%' . $wpdb->esc_like( $ex ) . '%' );
                }
            ...
            // Get excluded meta keys for exact matches
    ...
    /**
     * Get post type blacklist
     * @since 4.5.0
     */
    function get_post_types_blacklist() {
        ...
        return apply_filters( 'facetwp_post_types_blacklist', $post_types );
    }
}

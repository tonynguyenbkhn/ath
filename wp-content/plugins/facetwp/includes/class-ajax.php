...
class FacetWP_Ajax
{
    ...
    function backup() {
            ...
            if ( ! empty( $items ) ) {
                foreach ( $items as $item ) {
                    if ( 'facet' == substr( $item, 0, 5 ) ) {
                        ...
                        $output['facets'][] = FWP()->helper->get_facet_by_name( $item_name, 'all' );
                    }
        ...
        elseif ( 'import' == $action_type ) {
            ...
            $settings['facets'] = array_merge( $settings['facets_missing'], $settings['facets'] );
            unset( $settings['facets_missing'] );

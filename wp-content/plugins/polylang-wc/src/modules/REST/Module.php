...
class Module {
	...
	public function init() {
		...
		$this->product_category = new Translated\Term( PLL()->rest_api, 'product_cat', 'categories' );
		$this->product_tag      = new Translated\Term( PLL()->rest_api, 'product_tag', 'tags' );

		if ( taxonomy_exists( 'product_brand' ) ) {
			// WooCommerce Brands (WooCommerce 9.4+).
			$this->product_brand = new Translated\Term( PLL()->rest_api, 'product_brand', 'brands' );
		}
		...
		$this->filtered_attribute_term = new Filtered\Term( PLL()->rest_api, 'product_attribute_term' );
		$this->filtered_category       = new Filtered\Term( PLL()->rest_api, 'product_cat' );
		$this->filtered_product_tag    = new Filtered\Term( PLL()->rest_api, 'product_tag' );
		$this->filtered_brand          = new Filtered\Term( PLL()->rest_api, 'product_brand' );
	}

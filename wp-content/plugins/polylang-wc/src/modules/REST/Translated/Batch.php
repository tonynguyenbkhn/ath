...
trait Batch {
	 ...
	 */
	protected function check_batch_languages_permissions( Request $request ) {
		$items = array_merge(
			(array) $request->get_param( 'create' ),
			(array) $request->get_param( 'update' ),
		);

		$language_slugs = array_unique( array_column( $items, 'lang' ) );

		return Capabilities::get_user()->can_translate_all( $language_slugs );
	}
}

...
class Product extends Post {
	...
	private const OBJECT_TYPES = array( 'product', 'product_variation' );
	...
	public function __construct( PLL_REST_API $rest_api ) {
		parent::__construct( $rest_api, self::OBJECT_TYPES );


		$this->data_store = PLLWC_Data_Store::load( 'product_language' );

		foreach ( self::OBJECT_TYPES as $post_type ) {
		...
		add_filter( 'woocommerce_rest_check_permissions', array( $this, 'check_batch_permissions' ), 10, 4 );
	}
	 ...
	 * @since 2.2
	 *
	 * @throws WC_REST_Exception If the language is not valid or the user does not have permission to set the language.
	 *
	 * @param string     $lang   Language code.
	 * @param WC_Product $object Instance of `WC_Product`.
	 * @return bool|\WP_Error True when successfully assigned. False or WP_Error otherwise
	 *                        (or if the given language is already assigned to the object).
	 */
	public function set_language( $lang, $object ) {
		$language = parent::get_language_with_permission( $lang );
		if ( is_wp_error( $language ) ) {
			/** @var array<status, int> $data */
			$data = $language->get_error_data();
			/** @var int $status */
			$status = $data['status'];
			throw new WC_REST_Exception(
				esc_html( (string) $language->get_error_code() ),
				esc_html( $language->get_error_message() ),
				(int) $status,
			);
		}

		if ( $object instanceof WC_Product ) {
			return $this->data_store->set_language( $object->get_id(), $language );
		}

		return parent::set_language( $language->slug, $object );
	}
	...
	/**
	 * Checks if the user has permission to set the language of an object for batch requests.
	 *
	 * @since 2.2.2
	 *
	 ...
	 */
	public function check_batch_permissions( $permission, $context, $object_id, $object_type ) {
		if ( ! $permission ) {
